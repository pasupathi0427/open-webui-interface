<script lang="ts">
	...
	import {
		type Model,
		brandVariant,
		functions as localizedFunctions,
		mobile,
		settings,
		models,
		config,
									...
									<div class="flex items-center justify-between w-full">
										<div class="pl-[0.0625rem] flex items-center gap-2 text-sm dark:text-gray-500">
											<img
												...
												src={`${WEBUI_API_BASE_URL}/models/model/profile/image?id=${$models.find((model) => model.id === atSelectedModel.id).id}&lang=${$i18n.language}&theme=${$brandVariant}`}
											/>
