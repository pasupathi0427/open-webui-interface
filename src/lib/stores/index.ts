...
export const brandVariant = derived([theme, systemDarkMode], ([$theme, $systemDarkMode]) => {
	if ($theme === 'karix') return 'karix';
	if ($theme === 'dark' || $theme === 'oled-dark' || ($theme === 'system' && $systemDarkMode)) {
		return 'black';
	}
	return 'light';
});

export const brandLogoCircle = derived(
	brandVariant,
	($brandVariant) => `/static/karix-icons/${$brandVariant}-circle.svg`
);
export const brandLogoRounded = derived(
	brandVariant,
